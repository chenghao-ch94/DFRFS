...
class Trainer(object, metaclass=abc.ABCMeta):
    ...
    def try_evaluate(self, epoch):
        ...
        if self.train_epoch % args.eval_interval == 0:
            ...
            tvl, tva, tvap = self.evaluate(self.test_loader, epoch)
            self.logger.add_scalar('test_loss', float(tvl), self.train_epoch)
            self.logger.add_scalar('test_acc', float(tva),  self.train_epoch)
            print('epoch {}, test, loss={:.4f} acc={:.4f}+{:.4f}'.format(epoch, tvl, tva, tvap))

            if tva >= self.trlog['max_tacc']:
                self.trlog['max_tacc'] = tva
                self.trlog['max_tacc_interval'] = tvap
                self.trlog['max_tacc_epoch'] = self.train_epoch
                self.save_model('max_tacc')
    ...
    def save_model(self, name):
        torch.save(
            dict(params=self.para_model.state_dict()),
